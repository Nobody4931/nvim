...
return {
	{
		...
		keys = function(_plugin)
			...
			local keys = {
				{ "<leader>tg", function() other_toggle("g") end },
				{ "<leader>te", function() terminal_toggle(state.last) end },
				{ "<leader>tce", function() terminal_execute(state.last) end },
			}

			for i = 1, 9 do
				table.insert(keys, { "<leader>t" .. i, function() terminal_toggle(i) end })
				table.insert(keys, { "<leader>tc" .. i, function() terminal_execute(i) end })
			end
